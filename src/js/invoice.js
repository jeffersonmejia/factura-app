...
function printBill(form) {
	...
	const items = $clone.querySelectorAll('[id^="print-"]')

	const clientDNI = billData.client.dni,
		isFinalConsumer = clientDNI?.length < 1
	items.forEach((item) => {
	...
	const $listProductsPrint = $clone.getElementById('print-list-products'),
		$fragment = d.createDocumentFragment()
	billData.listProducts.forEach((el) => {
	...
	const printWindow = window.open(url, title, size),
